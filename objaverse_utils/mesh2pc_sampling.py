...
def sample_pc_features(glb_path, n_points):
    ...
    if isinstance(scene_or_mesh, trimesh.Scene):
        ...
        mesh = scene_or_mesh
    ## === Sample Position ===
    positions, face_indices = mesh.sample(n_points, return_index=True)
    ...
    for key, glb_lst in tqdm(data_dict.items()):
        ...
        for glb in tqdm(glb_lst):
            ...
            try:
                ...
                if SAVE_PLY:
                    save_pointcloud_as_ply(save_data['xyz'], save_data['rgb'], save_data['normal'],filename=out_path+'.ply')
