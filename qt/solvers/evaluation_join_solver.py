...
GOBJAVERSE_INDEX_PATH = 'datasets/json_files/objaverse_index_to_gobjaverse.json'
...
class EvaluationSolver(BaseSolver):
    ...
    def __init__(self, dm: QA3DBaseDataModule, solver: BaseSolver):
        ...
        self.index_map = None
        if self.dm.name == 'objaverse':
            with open(GOBJAVERSE_INDEX_PATH, 'r') as f:
                index_map = json.load(f)
                self.index_map = {key.replace('.glb', ''): val for key, val in index_map.items()}
    ...
    def on_validation_epoch_end(self):
        ...
        self._save_result_json(ids_all, self.criterion, score_pairs_list)
    ...
    def _save_result_json(self, ids: List[str], criterion: List[str], score_pairs_list):
        ## convert objaverse indices to gid
        if self.index_map is not None:
            ids = [x.replace('.npy', '') for x in ids]
            ids = [self.index_map[x] for x in ids]

        results = {
            'dataset': self.dm.name,
            'ids': ids,
            'criterion': criterion,
            'score_pairs': score_pairs_list,
        }

        save_path = os.path.join(self.output_dir, f'eval_results.json')
